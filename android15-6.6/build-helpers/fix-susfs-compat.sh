...
# v2.1.0 upstream wraps 'struct vm_area_struct *vma;' in #ifdef SUS_MAP
# inside pagemap_read(). On older sublevels the declaration and usage
# land in different block scopes, causing undeclared/unused errors.
# Fix: make the declaration unconditional.
# ---------------------------------------------------------------------------
if [ -f "$TMU" ]; then
